...
# 3. 模型路径
# 直接使用原始训练输出作为基座，不再复制：
# /root/autodl-tmp/ML/mixed1/outputs
model_dir="/root/autodl-tmp/ML/mixed1/outputs"
# 若需指定特定 ckpt，可填入；否则留空沿用目录内默认权重。
init_param=""

# 4. 数据路径
external_root="/root/autodl-tmp/ML/mixed-medium"
train_data="${external_root}/train_mixed_medium.jsonl"
val_data="${external_root}/val_mixed_medium_5k.jsonl"
